# Pubs Database — MySQL Version  
*(Microsoft SQL Server “Pubs” Sample Database Port)*

A fully compatible **MySQL 8.0+ port** of the classic **Microsoft SQL Server “Pubs” sample database**.  
This repository rebuilds the full schema, data, views, stored procedures, and triggers using MySQL syntax.

---

## Project Overview

The **Pubs** database was originally created by **Microsoft** as a teaching sample for SQL Server.  
It demonstrates essential relational database concepts such as:

- Table creation and referential constraints  
- Primary & foreign keys  
- Views, indexes, and stored procedures  
- Triggers and transaction control  

This repository adapts it for **MySQL 8.0**, maintaining the same logic and original data while ensuring syntax compatibility.

---

## Original Source

This project is based on Microsoft’s open-source SQL Server sample database:

- **Repository:** [microsoft/sql-server-samples](https://github.com/microsoft/sql-server-samples)
- **Original file:** [instpubs.sql](https://github.com/microsoft/sql-server-samples/blob/master/samples/databases/northwind-pubs/instpubs.sql)
- **License:** MIT License © Microsoft Corporation

---

## Disclaimer

This MySQL port of the Pubs database is provided **for learning, testing, and query practice only**.  
Some large or repetitive text objects from the original dataset have been **shortened or truncated** for simplicity.

---

# Pubs 数据库 — MySQL 版本  
*(Microsoft SQL Server “Pubs” 示例数据库移植版)*

这是微软经典示例数据库 **Pubs** 的 **MySQL 8.0+ 兼容移植版本**。  
该项目使用 MySQL 语法重建了原数据库的完整结构、数据、视图、存储过程以及触发器。

---

## 项目概述

**Pubs** 数据库最初由 **Microsoft** 创建，用于演示和教学 SQL Server 的核心特性。  
它展示了关系型数据库中的以下关键概念：

- 表的创建与外键约束  
- 主键与外键的关系  
- 视图、索引与存储过程  
- 触发器与事务控制  

本仓库将其移植为 **MySQL 8.0** 版本，在保持逻辑一致性与原始数据完整性的同时，确保语法兼容性。

---

## 原始来源

本项目基于微软开源的 SQL Server 示例数据库改写：

- **项目仓库：** [microsoft/sql-server-samples](https://github.com/microsoft/sql-server-samples)  
- **原始文件：** [instpubs.sql](https://github.com/microsoft/sql-server-samples/blob/master/samples/databases/northwind-pubs/instpubs.sql)  
- **许可证：** MIT License © Microsoft Corporation

---

## 声明

本 MySQL 移植版本仅供 **学习、测试与查询实验用途**。  
为简化体积与便于实验，原数据集中部分**冗长或重复的文本内容**已被**删减或保留示例性片段**。

---
